...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Matrix C; // Setup result structure
        C.rows = A.rows; C.cols = A.cols;
        C.data.resize(C.rows * C.cols);

        // Task 4: GPU time must include memory management + kernel execution
        auto start = std::chrono::high_resolution_clock::now();
        
        gpuMatrixAdd(A, B, C); // This function handles cudaMalloc, cudaMemcpy, and Kernel

        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> gpu_time = end - start;
        
        // Print ONLY the duration so the Python script can read it
        std::cout << gpu_time.count(); 
